        ...
        <>
            <div className='flex flex-col xl:flex-row xl:justify-between  text-[14px]  items-center m-[10px]'>
                <div className='flex justify-center items-center w-[150px] m-[20px]'>
                    <label className='mr-[5px]' htmlFor="category">Category:</label>
                    <select onChange={(e) => { setCategory(e.target.value) }} className='rounded-md border-[1px] border-gray-400 h-[30px]' name="category" id="category">
                ...
                <div className='flex flex-row items-center m-[30px] '>
                    <span onClick={() => { handlePrevious(setCurrentPage, currentPage, paginationNumbers) }} className='bg-gray-300 text-darkPurple rounded-md border-[1px] border-gray-400 w-[30px] h-[30px] flex justify-center items-center p-[5px]'>{icons.arrowLeft()}</span>
                    <div className='mx-[5px]'>
                        <span>page </span>
                        <input className='w-[30px] h-[30px] rounded-md border-[1px] border-gray-400 text-center' type="text" value={currentPage} onChange={(e) => { setCurrentPage(e.target.value) }}></input>
                        <span>{` from ${paginationNumbers}`}</span></div>
                    <span onClick={() => { handleNext(setCurrentPage, currentPage, paginationNumbers) }} className='bg-gray-300 text-darkPurple rounded-md border-[1px] border-gray-400 w-[30px] h-[30px] flex justify-center items-center p-[5px]'>{icons.arrowRight()}</span>
                </div>
            </div>
            <div className='text-[14px]'>
                ...
                </div>
                {
                    slicedData.map(({ id, title, category, time, view, comments }) => (
                        <PostItem key={id} id={id} title={title} category={category} time={time} view={view} comments={comments} />
                    ))
                }
            </div>
        </>
